...
<div class="my_account_component">
	<?php if (Yii::$app->user->isGuest) { ?>
		...
		<div class="not_logged_in flex flex-column">
			<span class="fpf_blacki fs1-143 fc-46"><?= Yii::t("app/old", "Not a member?") ?></span>
			<span class="fpf_i fs1 fc-9b"><?= Yii::t("app/old", "Register now and start enjoying Todevise!") ?></span>
			<br>
			<div class="funiv_bold fc-fff fs1-143 bc-c91c39 btn fs-upper"><?= Yii::t("app/old", "Join Todevise") ?></div>
			<br>

			<span class="fpf_blacki fs1-143 fc-46"><?= Yii::t("app/old", "ALREADY_MEMBER?") ?></span>
			<span class="fpf_i fs1 fc-9b"><?= Yii::t("app/old", "Login now!") ?></span>
			...
			<?= $form->field($login_model, 'email', [
				'inputOptions' => [
					...
					'placeholder' => Yii::t("app/old", "Email")
				]
			]) ?>
			...
			<div class="flex flex-justify-between flex-align-center relative">
				<?= Html::a(Yii::t("app/old", "Forgot password?"), ["global/forgot-password"], [
					'class' => 'fs-univ fc-6d fs0-857 forgot_password_label'
				]) ?>
				<?= $form->field($login_model, 'rememberMe', [
					'template' => "<span class='remember_me_label'>" . Yii::t("app/old", "Remember me") . "</span>{input}",
					'options' => [
			...
			<?= Html::submitButton(Yii::t('app/old', 'Login'), [
	...
	<?php } else { ?>
		...
		<div class="logged_in flex flex-column">
			<div class="profile_wrapper flex flex-justify-between">
				...
				<?= Html::a(Yii::t("app/old", "View profile"), ["deviser/view-profile"], [
					'class' => 'fs-univ fc-6d fs0-857 view_profile_label'
				]) ?>
			</div>
			<br>
			<?= Html::a(Yii::t("app/old", "My orders"), ["public/orders"], [
				'class' => 'fs-univ fc-6d fs0-857 text-right orders_label'
			]) ?>
			<?= Html::a(Yii::t("app/old", "Settings"), ["public/settings"], [
				'class' => 'fs-univ fc-6d fs0-857 text-right settings_label'
			]) ?>
			<?= Html::a(Yii::t("app/old", "Logout"), ["global/logout"], [
