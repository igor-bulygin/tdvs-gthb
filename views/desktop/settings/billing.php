			...
			<div uib-accordion-group class="panel-default panel-billing" heading="Connect with your stripe account" is-open="true" ng-cloak>
				<?php if (empty($person->settingsMapping->stripeInfoMapping->access_token)) { ?>
					...
					<div class="col-md-12 text-center">
						<a class="btn btn-medium btn-red" href="<?=Url::to(['settings/connect-stripe', 'slug' => $person->slug, 'person_id' => $person->short_id])?>"><?=Yii::t('app/public', 'CONNECT_WITH_STRIPE')?></a>
					</div>
				<?php } else { ?>
					<div><?=Yii::t('app/public', 'PROFILE_AND_STRIPE_LINKED_INFO')?></div>
					<br />
					<div class="col-md-12 text-center">
						<a class="btn btn-medium btn-red" href="<?=Url::to(['settings/connect-stripe', 'slug' => $person->slug, 'person_id' => $person->short_id])?>"><?=Yii::t('app/public', 'CONNECT_WITH_DIFFERENT_STRIPE_ACCOUNT')?></a>
					</div>
				<?php } ?>
			</div>
		</uib-accordion>
